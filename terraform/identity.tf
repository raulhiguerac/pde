...
#   service_account_id = google_service_account.service_account[var.service_accounts_binding[each.key].gsa-name].id
#   role               = "roles/iam.workloadIdentityUser"
#   members            = ["serviceAccount:${var.project}.svc.id.goog[${var.service_accounts_binding[each.key].gsa-name}/${var.service_accounts_binding[each.key].ksa-name}]"]
#   # depends_on         = [kubernetes_service_account.airflow_k8sa]
